...
export class HexKeyHelperPositions extends HexKey {
    props!:HexKeyProps
    ...
    getHexNodes = () => {
        ...
        this.props.positionStates.forEach((displayState:PositionState, index:number)=>{
            let node:JSX.Element = <HexNode   
                                    displayState = {new PositionState(true, true, true, PositionState.LABELTYPE_NUMBER) }
                                    onClick={()=>{}} 
                                    position={index} 
...
export default HexKeyHelperPositions